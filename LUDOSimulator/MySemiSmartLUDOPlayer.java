 ...
 *
 */
public class MySemiSmartLUDOPlayer implements LUDOPlayer {
	
	LUDOBoard board;
	Random rand;
	public MySemiSmartLUDOPlayer(LUDOBoard board)
	{
		this.board = board;
		rand = new Random();
	}
	...
	public float analyzeBrickSituation(int i) {
		if(board.moveable(i)) {
			...
			else if(board.isGlobe(new_board[board.getMyColor()][i]))
			{
				return 4;
			}
			else {
